...
uint32_t output_tick = 0; // 输出模式计时器
uint32_t key_tick = 0; // 按键按下计时器
uint8_t duty_lock_flag = 0; // 占空比锁定标志 0-未锁定 1-已锁定
...
void key_proc(void)
{
  ...
  {
    case 1:
      if(++lcd_disp_mode == 3) lcd_disp_mode = 0;
      if(lcd_disp_mode == 2)
      {
        set_index = 0;
        set_val_set[0] = set_val[0];
        set_val_set[1] = set_val[1];
      }
    break;
    case 2:
      if(lcd_disp_mode == 1) set_index ^= 1;
      else if(lcd_disp_mode == 0)
      {
  ...
  if(key_up == 4)
  {
    ...
    {
      if(lcd_disp_mode == 0 && duty_lock_flag == 0)
      {
        duty_lock_flag = 1;
      }
    }
    ...
    {
      if(lcd_disp_mode == 0 && duty_lock_flag == 1)
      {
        duty_lock_flag = 0;
      }
      else if(lcd_disp_mode == 1)
      {
        if(--set_val[set_index] == 0)
            set_val[set_index] = 10;
      }
    }
  }
