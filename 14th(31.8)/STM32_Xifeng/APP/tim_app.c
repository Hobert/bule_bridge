...
void pwm_set_frequency(int Frequency)
{
    ...
    uint32_t TIM2_CLK = 80000000;  // 例如72MHz, 需要根据实际情况调整
...
void ic_proc(void)
{
    ...
    speed_val = (tim_ic_val * 2 * 3.1415926 * set_val_set[0]) / 100 * set_val_set[1];
    ...
    {
        if(speed_val > speed_max[pwm_output_mode])
        {
            speed_max[pwm_output_mode] = speed_val;
        }
        speed_valid = 0;
    }
