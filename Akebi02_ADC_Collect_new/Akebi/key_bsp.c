...
typedef struct
{
  // uint32_t *r37_ptr = &r37;
  // uint32_t *r38_ptr = &r38;
...
param_ptr param_choose;

// 定义为“加”按键,每次按下，当前选择的参数加100
void keyB1_proc(button *btn, param_ptr param_choose)
{
  if (btn->id == 0 && flag_choose == 0)
  {
    param_choose.pd_ptr = &pd;
    *param_choose.pd_ptr += 100;
  }
  else if (btn->id == 0 && flag_choose == 1)
  {
    param_choose.ph_ptr = &ph;
    *param_choose.ph_ptr += 100;
  }
}

void keyB2_proc(button *btn, param_ptr param_choose)
{
  if (btn->id == 1 && flag_choose == 1)
  {
    param_choose.ph_ptr = &ph;
    *param_choose.ph_ptr -= 100;
  }
  else if (btn->id == 1 && flag_choose == 0)
  {
    param_choose.pd_ptr = &pd;
    *param_choose.pd_ptr -= 100;
  }
...
void keyB3_proc(button *btn)
{
  // 在参数界面下，按下B3按键，切换当前选择的参数(PD / PH)
  if (memu_choose == 1)
  {
    if (btn->id == 2)
    {
      flag_choose = (flag_choose + 1) % 2;
    }
  }

  // 在记录界面下，长按B3按键超过1秒后松开按键，清零该界面下的所有记录值，短按无效
  if (memu_choose == 2)
  {
    if (btn->id == 2)
    {
...
void keyB4_proc(button *btn)
{
  // 界面切换按键，按下B4按键可以往复切换数据、参数和记录三个界面
  if (btn->id == 3)
  {
    memu_choose_last = memu_choose;
    memu_choose = (memu_choose + 1) % 3; // 切换菜单0-1-2
                                         // 用于测试
    if (memu_choose != memu_choose_last)
    {
      LCD_Clear(Black);
    }
  }
...
uint8_t key_down = 0; // 记录按下的按键
uint8_t key_up = 0;   // 记录释放的按键
...
void key_proc(void)
{
  ...
  key_down = key_val & (key_old ^ key_val); // 为1的位为按下状态
  // 计算释放的按键（当前未按下状态与前一状态异或，并与前一状态相与）
  key_up = ~key_val & (key_old ^ key_val); // 为1的位为释放状态
  // 更新前一按键状态
  key_old = key_val;
...
void key_task(button *btn)
{
  ...
  case 2:                 // 按键释放状态
    if (btn->ticks >= 15) // 计时器达到阈值
    {
      ...
      if (btn->repeat == 1)
      {
        keyB1_proc(btn, param_choose); // 执行按键短按事件
        keyB2_proc(btn, param_choose); // 执行按键短按事件
        keyB3_proc(btn);               // 执行按键短按事件
        keyB4_proc(btn);               // 执行按键短按事件
      }
